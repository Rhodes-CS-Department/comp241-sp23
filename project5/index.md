...
The code you will write will be based on the quicksort code from class (see `quicksort-code.pdf` on Canvas), so you should base your code off of that. The only difference is that your code will use ArrayLists rather than plain arrays, and we will use a Comparator to let us sort things in different ways.

### What is a Comparator?

A Comparator is a Java class that implements the [Comparator](https://docs.oracle.com/en/java/javase/19/docs/api/java.base/java/util/Comparator.html) interface.  The Comparator interface has only  one method that must be implemented, which is `compare`.  The `compare` function in a Comparator is almost exactly like the `compareTo` function in a [Comparable](https://docs.oracle.com/en/java/javase/19/docs/api/java.base/java/lang/Comparable.html) object.  Here's the difference:
...
- ```
  ...
  ------------------------------------
  if (a < b)          if (comparator.compare(a, b) < 0)
  if (a > b)          if (comparator.compare(a, b) > 0)
  if (a <= b)         if (comparator.compare(a, b) <= 0)
  if (a >= b)         if (comparator.compare(a, b) >= 0)
  if (a == b)         if (comparator.compare(a, b) == 0)
...
- Write the `put()` function, which accepts a new key-value pair and adds it to the hash table. Notice that the specific hash index is already computed for you. (Hint: You will have to iterate through the linked list at the has index to see if the key is already in the hash table. You can do this using a for-each loop; e.g., `for (KVPair pair : list) { ...`.)
...
- Write `printTable()`.  Follow the instructions in the code.  Test in `HashtableMapTester`.
...
- Once you've written `getBestAutocomplete`, you can uncomment the call to `testAutocorrect()` in the `main` function of `AutocorrectDemo`. At this point, only the "Autocomplete" lines will work, but you can test your output against mine, for both `small.txt` and `nyt.txt`.
...
- Now you can write `getBestSuggestions()`. You will write this function in two phases.  For now, what you should inside this function is:
  ...
  - If the autocomplete string is not null (it might be null), then add it to the ArrayList as well, if it's not also in the set of autocorrect strings.
...
- Last step: We need to **sort** the autocomplete/autocorrect suggestions by word frequency.  To do this, we will write a `Comparator<String>` (I promised we'd come back to those!).  Head back to `Autocorrector.java`.  At the very end is a `Comparator` for Strings.  You must fill in the `compare` function to do a two-level comparison between the Strings s1 and s2, which will be words from your `wordFreq` map.  The basic idea is to compare s1 and s2 by their corresponding values in the map (which are, of course, how often each word appeared in the original text file.)  s1 should come "before" s2 if s1 is a more popular word (has a higher frequency) than s2.
  - Here's how to do it:
...
Feel free to play around with the GUI. To do this, uncomment the last two lines of `main` in `AutocorrectDemo.java`, and comment out the call to `testAutocorrect()`.
...
Upload the following four Java files to Canvas:
